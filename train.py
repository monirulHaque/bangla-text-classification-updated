...
from utils import get_task_config, parse_args, load_pickle
...
class Classification:
    def __init__(self, args):
        ...
        # data loaders
        train_data = load_pickle(self.config.train_file)
        val_data = load_pickle(self.config.val_file)
        test_data = load_pickle(self.config.test_file)

        train_dataset = BertDataset(train_data, self.config.sequence_len, args.model, self.config.num_class,
                                    self.config.balance)
        val_dataset = BertDataset(val_data, self.config.sequence_len, args.model, self.config.num_class,
                                  self.config.balance)
        test_dataset = BertDataset(test_data, self.config.sequence_len, args.model, self.config.num_class,
                                   self.config.balance)
        ...
        if MODELS[args.model][0] == BertModel or MODELS[args.model][0] == AutoModel:
            ...
            self.model = models.TextRoBERTa(pretrained_model=args.model, num_class=self.config.num_class,
                                            fine_tune=args.fine_tune)
        else:
    ...
    def run(self):
        ...
        pr, re, f1, _ = sklearn.metrics.precision_recall_fscore_support(y_true, y_pred, average='weighted')
